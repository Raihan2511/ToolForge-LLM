"""
scripts/eval_base_qwen.py
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
BFCL-Style Evaluation for BASE Qwen2.5-7B-Instruct (Unfine-tuned Baseline).

This script is a 1:1 replica of evaluation_latest.py with a single change:
the Inference Engine (Section 2) loads the BASE model directly via native
Hugging Face Transformers — NO PEFT / LoRA / Unsloth.

All evaluation logic (AST Static + Executable Dynamic) is byte-for-byte
identical to evaluation_latest.py so comparisons are scientifically valid.

Usage:
    python scripts/eval_base_qwen.py \
...
Inference stack:
  • Native Hugging Face Transformers (AutoModelForCausalLM) — NO PEFT.
  • 4-bit NF4 quantisation via BitsAndBytesConfig (double_quant, bfloat16 compute).
  • Standard KV-cache generation (use_cache=True) for maximum inference speed.
  • ChatML template is applied via tokenizer.apply_chat_template().

Author: Auto-generated evaluation pipeline (base model variant)
Python: 3.10+
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
"""
...
# NOTE: No peft or unsloth imports — this evaluates the base model directly.
...
logger = logging.getLogger("eval_base_qwen")
...
def parse_args() -> argparse.Namespace:
    """Parse command-line arguments for the base model evaluation pipeline."""
    parser = argparse.ArgumentParser(
        description="BFCL-style evaluation for BASE Qwen2.5-7B-Instruct (no fine-tuning)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument(
        "--model_path",
        type=str,
        # ── KEY CHANGE: Default to the base Qwen model from Hugging Face Hub ──
        default="Qwen/Qwen2.5-7B-Instruct",
        help="Hugging Face model ID or local path to the BASE model (default: Qwen/Qwen2.5-7B-Instruct).",
    )
    parser.add_argument(
        "--dataset_path",
        type=str,
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# SECTION 2 — Inference Engine (Native Hugging Face — NO PEFT / NO Unsloth)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
def load_model(model_path: str, max_seq_length: int = 2048):
    """
    Load the BASE Qwen2.5-7B-Instruct model using native Hugging Face.

    This is the key difference from evaluation_latest.py:
      • NO adapter_config.json lookup
      • NO PeftModel overlay
      • NO Unsloth FastLanguageModel
      The model is loaded directly in 4-bit NF4 quantisation.

    Pipeline:
      1. Configure 4-bit NF4 quantisation via BitsAndBytesConfig.
      2. Load the base model directly from the HF Hub / local path.
      3. Load the tokenizer with left-padding for generation alignment.
      4. Set model to eval mode.

    Args:
        model_path: Hugging Face model ID (e.g. "Qwen/Qwen2.5-7B-Instruct")
                    or local path to the base model.
        max_seq_length: Maximum sequence length (used for tokenizer consistency).

    Returns:
        Tuple of (model, tokenizer).
    """
    # ── 4-bit quantisation config (identical to evaluation_latest.py) ──
    ...
    # ── Load base model directly — no adapter overlay ──
    logger.info(f"Loading BASE model: {model_path} (4-bit NF4)...")
    ...
    # ── Inference mode — plain eval(), no adapter merging needed ──
    model.eval()
    logger.info("Base model loaded and set to eval mode (native HF, no PEFT)")
...
def generate(
    ...
    Uses standard Transformers generation with use_cache=True for maximum
    inference speed.
    ...
    with torch.no_grad():
        output_ids = model.generate(
            ...
            use_cache=True,               # ── KV cache enabled for fast inference ──
            # use_cache=True is safe and fast for the native HF base model.
            # No Unsloth Triton kernel issues to worry about here.
        )
...
def run_pipeline(
    ...
    for idx, sample in enumerate(tqdm(samples, desc="🔄 Evaluating (Base)", unit="sample")):
...
def print_metrics(metrics: dict[str, Any]) -> None:
    ...
    W = 70
    print("\n" + "━" * W)
    print("  📊  EVALUATION RESULTS — Base Qwen2.5-7B-Instruct (No Fine-Tuning)")
    print("━" * W)
...
def save_results(
    ...
    with open(summary_file, "w", encoding="utf-8") as f:
        f.write("BFCL-Style Evaluation Summary — BASE Qwen2.5-7B-Instruct\n")
        f.write("=" * 60 + "\n\n")
...
def main() -> None:
    ...
    # ── Banner ──
    print("\n" + "━" * 70)
    print("  🧪  BFCL-Style Evaluation — BASE Qwen2.5-7B-Instruct (Baseline)")
    print("━" * 70)
    ...
    # ── Step 2: Load model ──
    logger.info("Step 2/3: Loading BASE model...")
